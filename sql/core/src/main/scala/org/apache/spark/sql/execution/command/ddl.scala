...
import org.apache.spark.sql.execution.datasources.{HadoopFsRelation, LogicalRelation, PartitioningUtils, SymlinkTextInputFormatUtil}
    ...
    if (table.stats.nonEmpty) {
      if (sparkSession.sessionState.conf.autoSizeUpdateEnabled) {
        val addedSize = CommandUtils.calculateTotalLocationSize(sparkSession, table.identifier,
          parts.map(_.storage.locationUri), SymlinkTextInputFormatUtil.isSymlinkTextFormat(table))
        if (addedSize > 0) {
